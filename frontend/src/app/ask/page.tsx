...
export default function AskPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-6 shell:px-8">
      <h1 className="font-mono text-[11px] tracking-[0.26em] text-terminal-up">
        AGENT QUERY · ASK THE BENCH
      </h1>
      <p className="mt-2 font-body text-[13px] text-ink-muted">
        Parallel LLM chambers — log every trace for post-hoc eval.
      </p>
      ...
      <textarea
        ...
        rows={5}
        className="mt-8 w-full resize-y rounded-xs border border-terminal-line bg-terminal-panel px-4 py-3 font-body text-[14px] text-ink placeholder:text-ink-faint focus:border-terminal-up/45 focus:outline-none"
      />
      ...
      <button
        ...
        disabled={loading}
        onClick={submit}
        className="mt-6 rounded-xs border border-terminal-up/45 bg-terminal-up/12 px-6 py-3 font-mono text-[12px] tracking-[0.2em] text-terminal-up disabled:opacity-50"
      >
